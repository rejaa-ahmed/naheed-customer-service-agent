...
class TestRetryLogic(unittest.TestCase):
    ...
    def test_one_message_one_call(self):
        self.mock_client.generate_content.return_value = '{"intent": "greeting", "confidence": 0.95, "entities": {}, "tool": null}'
        
        self.cm.process_message("hello")
        
        # Verify exactly one call to the LLM per user request
        self.assertEqual(self.mock_client.generate_content.call_count, 1)
        self.mock_router.route.assert_not_called()
        
    def test_router_fallback_on_429(self):
        self.mock_client.generate_content.side_effect = GeminiAPIError("API request failed: 429 RESOURCE_EXHAUSTED")
    ...
    def test_router_fallback_on_timeout(self):
        self.mock_client.generate_content.side_effect = GeminiAPIError("API request failed: 504 Deadline Exceeded")
        self.mock_router.route.return_value = IntentResult(intent="unknown", confidence=1.0, entities={}, tool=None)
    ...
    def test_router_fallback_on_malformed_json(self):
        self.mock_client.generate_content.return_value = "This is not JSON at all."
        self.mock_router.route.return_value = IntentResult(intent="unknown", confidence=1.0, entities={}, tool=None)
